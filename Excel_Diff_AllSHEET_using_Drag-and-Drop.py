...
# 日本語ファイル名対応のため、ファイルパスをUnicodeにする
file1abs = os.path.abspath(file1)
file2abs = os.path.abspath(file2)
print(file1abs)
print(file2abs)

input("pd.ExcelFile前    Enterを押すと続行します...")
# Excelファイルのすべてのシート名を取得
xls1 = pd.ExcelFile(file1)
xls2 = pd.ExcelFile(file2)

input("pd.ExcelFile後     Enterを押すと続行します...")
        
# 差分リスト
all_diffs = []

# すべてのシートを比較
for sheet_name in set(xls1.sheet_names).union(set(xls2.sheet_names)):
    print(f"シート: {sheet_name} を比較中...")
    
    # 各シートのデータを読み込み
    df1 = pd.read_excel(file1, sheet_name=sheet_name, header=None) if sheet_name in xls1.sheet_names else pd.DataFrame()
    df2 = pd.read_excel(file2, sheet_name=sheet_name, header=None) if sheet_name in xls2.sheet_names else pd.DataFrame()
...
if all_diffs:
    ...
    diff_df = pd.DataFrame(all_diffs, columns=["シート名", "セル", "ファイル1の値", "ファイル2の値"])
...
#           input("正常に「.xlsx」が作成されました。\nPress Enter to Exit...")
        ...
        input("DEBUG END    Enterを押すと続行します...")
else:
    print("差分は見つかりませんでした。")
    input("DEBUG END    Enterを押すと続行します...")
